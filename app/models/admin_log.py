...
class AdminLog(db.Model):
    ...
    admin_id = db.Column(db.Integer, db.ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
    action = db.Column(db.String(255), nullable=False)
    target_report_id = db.Column(db.Integer, db.ForeignKey("crime_reports.id", ondelete="SET NULL"), nullable=True, index=True)
